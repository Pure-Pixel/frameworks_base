...
final class GsmSMSDispatcher extends SMSDispatcher {
    ...
    protected void handleBroadcastSms(AsyncResult ar) {
        try {
            ...
            if(isUMTSPdu(receivedPdu)){
            	if(isUMTSCbsMessage(receivedPdu)){
    ...
    private boolean isUMTSPdu(byte[] pdu){
   		return pdu.length > 88 && pdu.length <= 1252;
    }
    
    private boolean isUMTSCbsMessage(byte[] pdu){
    	// at UMTS networks the CB header includes in the first octet the message type information element.
    	// 1 - CBS Message
    	// 2 - Schedule Message
    	// 3 - CBS41 Message
    	// this part handle only pdus which contain CBS message for UMTS (message type = 1)
    	// according to 3GPP 25.324 
       	return pdu[0] == 1;
    }
