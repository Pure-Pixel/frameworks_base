...
public class SipMessageParsingUtils {
    ...
    // The Via branch parameter must start with this cookie if it is SIP 2.0.
    ...
    public static String getTransactionId(String headerString) {
        ...
        for (Pair<String, String> header : headers) {
            ...
            String[] params = header.second.split(PARAM_SEPARATOR);
            if (params.length < 2) {
            ...
            for (String param : params) {
                ...
                if (pair.length > 2) {
                    Log.w(TAG, "getTransactionId: unexpected parameter" + Arrays.toString(pair));
                }
                if (BRANCH_PARAM_KEY.equalsIgnoreCase(pair[0])
                        && pair[1].startsWith(BRANCH_MAGIC_COOKIE)) {
    ...
    private static List<Pair<String, String>> parseHeaders(String headerString,
        ...
        headerLines[0] = removeLeadingWhitespace(headerLines[0]);
        ...
        for (String line : headerLines) {
            ...
            if (line.startsWith("\t") || line.startsWith(" ")) {
                headerValueSegment.append(line);
                continue;
            }
