...
/**
 * A wrapper template class for session handlings for Drm Engines. It wraps a pointer type
 * over KeyedVector. It keeps pointer as data in the vector and free up memory allocated
 * pointer can be of any type of structure/class meant for keeping session data.
...
/**
 * returns the number of objects in the sessio map table
 *
...
/**
 * empty the entire session table. If releases all the memory for session objects.
 */
void destroyMap() {
    int size = map.size();
    int i = 0;
