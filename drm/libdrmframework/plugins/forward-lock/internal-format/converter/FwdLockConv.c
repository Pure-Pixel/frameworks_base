 ...
 * @return A session ID.
 */
static int FwdLockConv_AcquireSession(void) {
    int sessionId = -1;
    int i;
 ...
 *
 * @param pSession A reference to a converter session.
 *
...
static int FwdLockConv_DeriveKeys(FwdLockConv_Session_t *pSession) {
    ...
    if (pData == NULL) {
        ...
        if (AES_set_encrypt_key(pSession->sessionKey, KEY_SIZE_IN_BITS,
            ...
            memset(pData->value, 0, sizeof pData->value);
            AES_encrypt(pData->value, pData->key, &pData->sessionRoundKeys);
            if (AES_set_encrypt_key(pData->key,
                                    KEY_SIZE_IN_BITS,
...
/**
 * Checks whether a given character is valid in a boundary.
 *
 * @param ch The character to check.
 *
 * @return
 *  A Boolean value indicating whether the given character is valid as the last one in a boundary.
 */
...
static FwdLockConv_Status_t FwdLockConv_MatchOpenDelimiter(FwdLockConv_Session_t *pSession,
    ...
    case FwdLockConv_ScannerState_WantsBoundary:
        if (FwdLockConv_IsBoundaryChar(ch)) {
            if (pSession->delimiterLength < sizeof pSession->delimiter) {
 ...
 *
 * @param pString A reference to a dynamically growing string.
 ...
 *
 * @param pSession A reference to a converter session.
 * @param pOutput The output from the conversion process (allocated).
 *
 * @return A status code.
 */
...
static void FwdLockConv_IncrementCounter(FwdLockConv_Session_t *pSession) {
    size_t i = 0;
    while ((++pSession->counter[i] == 0) && (++i < sizeof pSession->counter));
}
 ...
 *
 * @param pSession A reference to a converter session.
 * @param ch The character to encrypt and write.
 * @param pOutput The output from the conversion process (reallocated, if needed).
 *
 * @return A status code.
 */
static FwdLockConv_Status_t FwdLockConv_WriteEncryptedChar(FwdLockConv_Session_t *pSession,
    ...
    if (++pSession->keyStreamIndex == sizeof pSession->keyStream) {
 ...
 * During this, a signature of the encrypted data is calculated.
 *
 * @param pSession A reference to a converter session.
 * @param ch A character.
 * @param pOutput The output from the conversion process (reallocated, if needed).
 ...
 * @param pSession A reference to a converter session.
 * @param ch A character.
 * @param pOutput The output from the conversion process (allocated/reallocated).
 *
 * @return A status code.
 */
    ...
    if (pSessionId == NULL || pOutput == NULL) {
        ...
        *pSessionId = FwdLockConv_AcquireSession();
        if (*pSessionId < 0) {
            ...
            if (pSession->encryptedSessionKeyLength < sizeof pSession->counter) {
                // The encrypted session key is used as the CTR-mode nonce,
                // so it must be at least the size of a single AES block.
                status = FwdLockConv_Status_ProgramError;
            } else {
                ...
                if (pSession->pEncryptedSessionKey == NULL) {
                    ...
                    if (!FwdLockGlue_GetRandomNumber(pSession->sessionKey,
                                                     sizeof pSession->sessionKey)) {
                        status = FwdLockConv_Status_RandomNumberGenerationFailed;
                    } else if (!FwdLockGlue_EncryptKey(pSession->sessionKey,
                                                       sizeof pSession->sessionKey,
                    ...
                    if (status == FwdLockConv_Status_OK) {
                        // Zero out key data.
                        memset(pSession->sessionKey, 0, sizeof pSession->sessionKey);
                        memcpy(pSession->counter, pSession->pEncryptedSessionKey,
                               sizeof pSession->counter);
    ...
    if (!FwdLockConv_IsValidSession(sessionId) || pOutput == NULL) {
        ...
        if (pSession->parserState != FwdLockConv_ParserState_Done) {
            ...
            if (signatureSize != SHA1_HASH_SIZE) {
                ...
                // Calculate the header signature, which is a signature of
                // the rest of the header including the data signature.
                ...
                if (signatureSize != SHA1_HASH_SIZE) {
                    ...
                    pOutput->fromCloseSession.fileOffset =
                        TOP_HEADER_SIZE + pSession->contentType.length +
                        pSession->encryptedSessionKeyLength;
                    status = FwdLockConv_Status_OK;
                }
            }
    ...
    if (fpReadFunc == NULL || fpWriteFunc == NULL || fpLSeekFunc == NULL || inputFileDesc < 0 ||
        ...
        if (pReadBuffer == NULL) {
            ...
            if (status == FwdLockConv_Status_OK) {
                ...
                while ((numBytesRead = fpReadFunc(inputFileDesc,
                                                  pReadBuffer, READ_BUFFER_SIZE)) > 0) {
                    status = FwdLockConv_ConvertData(sessionId, pReadBuffer,
