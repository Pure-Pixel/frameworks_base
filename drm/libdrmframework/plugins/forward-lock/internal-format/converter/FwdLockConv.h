...
#ifndef INCLUSION_GUARD_FWDLOCKCONV
#define INCLUSION_GUARD_FWDLOCKCONV
...
/**
 * The size of the data and header signatures combined. The signatures are adjacent
 * to each other inthe produced output file.
 */
#define FWD_LOCK_SIGNATURES_SIZE 40

/**
 * Data type for the output from FwdLockConv_ConvertData.
 */
typedef struct FwdLockConv_ConvertData_Output {
    void *pBuffer; ///< The converted data.
    size_t numBytes; ///< The size of the converted data.
    off_t errorPos; ///< The file position where the error occurred, in the case of a syntax error.
...
typedef struct FwdLockConv_CloseSession_Output {
    unsigned char signatures[FWD_LOCK_SIGNATURES_SIZE]; ///< The final set of signatures.
    off_t fileOffset; ///< The offset in the produced output file where the signatures are located.
    off_t errorPos; ///< The file position where the error occurred, in the case of a syntax error.
 ...
 */
typedef off_t FwdLockConv_LSeekFunc_t(int fileDesc, off_t offset, int whence);
 ...
 * file format. The caller must update the produced output file at the indicated file offset
 * with the final set of signatures.
 *
 * @param[in] sessionId The session ID.
...
#endif // INCLUSION_GUARD_FWDLOCKCONV
