...
#define INVALID_BLOCK_INDEX ((size_t)-1)
...
static int FwdLockFile_FindSession(int fileDesc) {
    int i;
    for (i = 0; i < MAX_NUM_SESSIONS; ++i) {
        int sessionId = (fileDesc + i) % MAX_NUM_SESSIONS;
        if (sessionPtrs[sessionId] != NULL && sessionPtrs[sessionId]->fileDesc == fileDesc) {
            return sessionId;
        }
    }
    errno = EBADF;
 ...
 *
 * @param pSession A reference to a file session.
 *
...
/**
 * Calculates the counter, treated as a 16-byte little-endian number,
 * used to generate the keystream for the given block.
 *
 * @param pNonce  A reference to the nonce.
 * @param blockIndex  The index number of the block.
 * @param pCounter  A reference to the counter.
 */
 ...
 *
 * @param pSession A reference to a file session.
 * @param pByte The byte to decrypt.
 */
void FwdLockFile_DecryptByte(FwdLockFile_Session_t * pSession, unsigned char *pByte) {
    size_t blockIndex = pSession->filePos / AES_BLOCK_SIZE;
    size_t blockOffset = pSession->filePos % AES_BLOCK_SIZE;
...
int FwdLockFile_attach(int fileDesc) {
    int sessionId;
    sessionId = FwdLockFile_AcquireSession(fileDesc);
    if (sessionId >= 0) {
        ...
        if (read(fileDesc, pSession->topHeader, TOP_HEADER_SIZE) == TOP_HEADER_SIZE &&
            ...
            if (pSession->pContentType != NULL &&
                ...
                if (pSession->pEncryptedSessionKey != NULL &&
                    ...
                    read(fileDesc, pSession->dataSignature, SHA1_HASH_SIZE) == SHA1_HASH_SIZE &&
                    read(fileDesc, pSession->headerSignature, SHA1_HASH_SIZE) == SHA1_HASH_SIZE) {
...
off_t FwdLockFile_lseek(int fileDesc, off_t offset, int whence) {
    off_t newFilePos;
    int sessionId = FwdLockFile_FindSession(fileDesc);
    if (sessionId < 0) {
        ...
        switch (whence) {
        case SEEK_SET:
            newFilePos = lseek(pSession->fileDesc, pSession->dataOffset + offset, whence);
            break;
        case SEEK_CUR:
        case SEEK_END:
            newFilePos = lseek(pSession->fileDesc, offset, whence);
            break;
        default:
            errno = EINVAL;
            newFilePos = INVALID_OFFSET;
        ...
        if (newFilePos != INVALID_OFFSET) {
            if (newFilePos < pSession->dataOffset) {
                // The new file position is illegal for an internal Forward Lock file.
                // Restore the original file position.
                (void)lseek(pSession->fileDesc, pSession->dataOffset + pSession->filePos,
                            SEEK_SET);
                errno = EINVAL;
                newFilePos = INVALID_OFFSET;
            } else {
...
int FwdLockFile_CheckDataIntegrity(int fileDesc) {
    ...
    if (sessionId < 0) {
        ...
        if (pData == NULL) {
            ...
            FwdLockFile_Session_t *pSession = sessionPtrs[sessionId];
            if (lseek(pSession->fileDesc, pSession->dataOffset, SEEK_SET) !=
                    pSession->dataOffset) {
                result = FALSE;
            } else {
                ssize_t numBytesRead;
                size_t signatureSize = sizeof pData->signature;
                while ((numBytesRead =
                            read(pSession->fileDesc, pData->buffer, sizeof pData->buffer)) > 0) {
                ...
                if (numBytesRead < 0) {
                    ...
                    assert(signatureSize == sizeof pData->signature);
...
int FwdLockFile_CheckHeaderIntegrity(int fileDesc) {
    ...
    if (sessionId < 0) {
        ...
        assert(signatureSize == sizeof signature);
