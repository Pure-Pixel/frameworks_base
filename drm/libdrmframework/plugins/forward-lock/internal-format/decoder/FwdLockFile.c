    ...
    if (sessionId < 0) {
        ...
        if (newFilePos != INVALID_OFFSET) {
            if (newFilePos < pSession->dataOffset) {
                ...
                (void)lseek(pSession->fileDesc, pSession->dataOffset + pSession->filePos, SEEK_SET);
                errno = EINVAL;
                newFilePos = INVALID_OFFSET;
            } else {
                // The return value should be the file position that lseek() would have returned for
                // the embedded content file.
...
int FwdLockFile_CheckDataIntegrity(int fileDesc) {
    ...
    if (sessionId < 0) {
        ...
        if (pData == NULL) {
            ...
            FwdLockFile_Session_t *pSession = sessionPtrs[sessionId];
            if (lseek(pSession->fileDesc, pSession->dataOffset, SEEK_SET) != pSession->dataOffset) {
                result = FALSE;
            } else {
                ssize_t numBytesRead;
