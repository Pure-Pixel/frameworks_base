...
static int FwdLockGlue_CreateDirectories(const char *path, mode_t mode) {
    int result = TRUE;
    size_t partialPathLength;
    partialPathLength = strlen(path);
...
int FwdLockGlue_GetRandomNumber(void *pBuffer, size_t numBytes) {
    // Cryptographically generated random bytes by reading them from "/dev/urandom" (the
    // non-blocking version of "/dev/random").
    ssize_t numBytesRead = 0;
    int fileDesc;
    fileDesc = open("/dev/urandom", O_RDONLY);
    if (fileDesc < 0) {
        numBytesRead = 0;
    } else {
...
int FwdLockGlue_InitializeKeyEncryption() {
    static pthread_mutex_t initializationMutex = PTHREAD_MUTEX_INITIALIZER;
    static int isInitialized = FALSE;
    pthread_mutex_lock(&initializationMutex);
    if (!isInitialized) {
    ...
    pthread_mutex_unlock(&initializationMutex);
