...
public class SubscriptionView extends LinearLayout {
    ...
    public void setSubInfo(SubInfoRecord subInfo) {
        if (subInfo != null) {
            setSubColor(subInfo.mSimIconRes[mThemeType]);
            setSubName(subInfo.mDisplayName);
            setSubNum(subInfo.mNumber);
            setSubShortNum(subInfo.mDispalyNumberFormat, subInfo.mNumber);
        }
    }
    ...
    public void setSubShortNum(int format, String num) {
        if (num != null) {
            ...
            if (num.length() <= mNumLength) {
                ...
                if (format != SubscriptionController.DISPALY_NUMBER_NONE) {
                    formatNum = format == SubscriptionController.DISPLAY_NUMBER_FIRST ? 
