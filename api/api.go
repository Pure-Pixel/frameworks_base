...
func (a *CombinedApis) ConvertWithBp2build(ctx android.TopDownMutatorContext) {
	if ctx.ModuleType() != "combined_apis" {
		return
	}
	...
	for scopeName, suffix := range scopeToSuffix {
		...
		if scopeName == "system-server" {
			deps = bazel.MakeLabelListAttribute(android.BazelLabelForModuleDeps(ctx, a.properties.System_server_classpath))
		} else {
			deps = bazel.MakeLabelListAttribute(android.BazelLabelForModuleDeps(ctx, a.properties.Bootclasspath))
		}
